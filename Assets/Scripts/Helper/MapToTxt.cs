...
public class MapToTxt 
{
    ...
    public MapToTxt(string folderName)
    {
        this.folderName = folderName;
        if (folderName == "")
            folderName = "Unknow";
        FileHelper.CreateFolder(FileHelper.testFolder, folderName);
    }
    ...
    public void ThrowLogToFile(string alias, string log)
    {
        if (Do)
        {
            string filePath;
            if (alias != "")
                filePath = Path.Combine(FileHelper.testFolder, folderName, $"Log{alias}.txt");
            else
                filePath = Path.Combine(FileHelper.testFolder, $"Log{alias}.txt");
    ...
    public float[,] LoadHeightFromFile(string filename)
    {
        float[,] array = null;
        string filePath = Path.Combine(FileHelper.testFolder, filename);
        string line;
    ...
    public void HeightToFile(float[,] heightMap, string alias = "",string format = "F4", bool MapMode = false)
    {
        ...
        if(Do)
        {
            string filePath = Path.Combine(FileHelper.testFolder, $"floatArray2D{alias}.txt");
            try
            {
                using (StreamWriter sw = new StreamWriter(filePath))
                {
    ...
    public void VectorsToFile(Vector3[] array)
    {
        if (Do)
        {
            string filePath = Path.Combine(FileHelper.testFolder, "Vectors.txt");
    ...
    public void HeightToBool(float[,] heightMap, float min, float max)
    {
        if (Do)
        {
            string filePath = Path.Combine(FileHelper.testFolder, "HeightBool.txt");
            heightMap = ArrayModify.ArrayReverseY(heightMap);
            heightMap = ArrayModify.ArrayCutXY(heightMap);
            heightMap = ArrayModify.ArrayReverseX(heightMap);
            try
    ...
    public void GenerateRabbitFields(float[,] array , float min, float max, int population)
    {
        if (Do)
        {
            ...
            int fields = Ax * Ay;
            string filePath = Path.Combine(FileHelper.testFolder, "Rabbits.txt");
    ...
    public void ReadFloatArray2D(float[,] array)
    {
        if (Do)
        {
            string filePath = Path.Combine(FileHelper.testFolder, "Reader.txt");
            using (StreamWriter sw = new StreamWriter(filePath))
            {
    ...
    public void ReadMapArray2D(char[,] array, string alias = "")
    {
        string filePath;
        if (alias != "")
            filePath = Path.Combine(FileHelper.testFolder, folderName,$"Map{alias}.txt");
        else
